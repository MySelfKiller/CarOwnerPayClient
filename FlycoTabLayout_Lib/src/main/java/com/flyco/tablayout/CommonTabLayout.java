...
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    private static final int STYLE_STORKE = 3;
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mIndicatorColor = ta.getColor(R.styleable.CommonTabLayout_tl_indicator_color, Color.parseColor((mIndicatorStyle == STYLE_BLOCK || mIndicatorStyle == STYLE_STORKE) ? "#4B6A87" : "#ffffff"));
        mIndicatorHeight = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_height,
                dp2px(mIndicatorStyle == STYLE_TRIANGLE ? 4 : ((mIndicatorStyle == STYLE_BLOCK || mIndicatorStyle == STYLE_STORKE)? -1 : 2)));
        mIndicatorWidth = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_width, dp2px(mIndicatorStyle == STYLE_TRIANGLE ? 10 : -1));
        mIndicatorCornerRadius = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_corner_radius, dp2px((mIndicatorStyle == STYLE_BLOCK || mIndicatorStyle == STYLE_STORKE) ? -1 : 0));
        mIndicatorMarginLeft = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_margin_left, dp2px(0));
        mIndicatorMarginTop = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_margin_top, dp2px((mIndicatorStyle == STYLE_BLOCK || mIndicatorStyle == STYLE_STORKE) ? 0 : 0));
        mIndicatorMarginRight = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_margin_right, dp2px(0));
        mIndicatorMarginBottom = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_margin_bottom, dp2px((mIndicatorStyle == STYLE_BLOCK || mIndicatorStyle == STYLE_STORKE) ? 0 : 0));
    ...
    private String[] title2Array ;
    private String[] titleArray ;
    /** 创建并添加tab */
    private void addTab(final int position, View tabView) {

        TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
        TextView tv_tab_2title = (TextView) tabView.findViewById(R.id.tv_tab_title_2);
        tv_tab_title.setText(Html.fromHtml(mTabEntitys.get(position).getTabTitle()));
        String title2 = mTabEntitys.get(position).getTabTitle2();
        if (null != title2 && !"".equals(title2)){
            tv_tab_2title.setText(title2);
            tv_tab_2title.setVisibility(VISIBLE);
        } else {
                tv_tab_2title.setVisibility(GONE);
        }
        ImageView iv_tab_icon = (ImageView) tabView.findViewById(R.id.iv_tab_icon);
        iv_tab_icon.setImageResource(mTabEntitys.get(position).getTabUnselectedIcon());
        iv_tab_icon.setScaleType(ImageView.ScaleType.FIT_XY);
    ...
    private void updateTabStyles() {
        for (int i = 0; i < mTabCount; i++) {
            ...
            tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX,
                    i == mCurrentTab ? mTextsize : mTextsize-5);
//            tv_tab_title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
            ...
            if (null != title2Array && title2Array.length == mTabCount){
                if (null != title2Array[i] && !"".equals(title2Array[i])){
                    tv_tab_2title.setText(title2Array[i]);
                }
            }
            if (null != titleArray && titleArray.length == mTabCount){
                if (null != titleArray[i] && !"".equals(titleArray[i])){
                    tv_tab_title.setText(titleArray[i]);
                }
            }

            if (tv_tab_2title.getVisibility() == VISIBLE){
                tv_tab_2title.setTextColor(i == mCurrentTab ? mTextSelectColor : mTextUnselectColor);
                tv_tab_2title.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextsize);
//            tv_tab_2title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
                if (mTextAllCaps) {
                    tv_tab_2title.setText(tv_tab_2title.getText().toString().toUpperCase());
                }

                if (mTextBold == TEXT_BOLD_BOTH) {
                    tv_tab_2title.getPaint().setFakeBoldText(true);
                } else if (mTextBold == TEXT_BOLD_NONE) {
                    tv_tab_2title.getPaint().setFakeBoldText(false);
                }
            }
...
//            if (mIndicatorStyle == STYLE_STORKE){
...
//                    back.setStroke(dp2px(1),mTextUnselectColor);
//                    back.setColor(Color.parseColor("#FFFFFF"));
//                    back.setBounds(paddingLeft + (int) mIndicatorMarginLeft + mIndicatorRect.left,
//                            (int) mIndicatorMarginTop, (int) (paddingLeft + mIndicatorRect.right - mIndicatorMarginRight),
//                            (int) (mIndicatorMarginTop + mIndicatorHeight));
//                    back.setCornerRadius(mIndicatorCornerRadius);
//                    back.setGradientType(GradientDrawable.RECTANGLE);
//                    tabView.setBackgroundDrawable(back);
//                }
//            }
        }
    }
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            TextView tab_2title = (TextView) tabView.findViewById(R.id.tv_tab_title_2);
            tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, isSelect ? mTextsize : mTextsize-5);
            tab_title.setTextColor(isSelect ? mTextSelectColor : mTextUnselectColor);
            tab_2title.setTextColor(isSelect ? mTextSelectColor : mTextUnselectColor);
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIndicatorStyle == STYLE_TRIANGLE) {
        ...
        } else if (mIndicatorStyle == STYLE_STORKE){
            if (mIndicatorHeight < 0) {
                mIndicatorHeight = height - mIndicatorMarginTop - mIndicatorMarginBottom;
            } else {
            ...
            if (mIndicatorHeight > 0) {
                ...
                mIndicatorDrawable.setStroke(dp2px(1),mIndicatorColor);
    ...
    public void setTitle2(String[] title2Array) {
        this.title2Array = title2Array;
        updateTabStyles();
    }
    public void setTitle(String[] titleArray) {
