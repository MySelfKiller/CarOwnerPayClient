 ...
 */
public class WashOrderListDataParser extends BaseParse {
    @Override
        ...
        if (state == 1) {
            ...
            if (null != dataJson){
                ...
                if (null != listJson && listJson.length() > 0) {
                    ArrayList<ItemWashOrderBean> listArray = new ArrayList<ItemWashOrderBean>();
                    for (int x = 0; x < listJson.length(); x++) {
                        ItemWashOrderBean stationBean = GsonHelper.fromJson(listJson.get(x).toString(), ItemWashOrderBean.class);
